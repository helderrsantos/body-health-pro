...
async function renderSingleEvaluationPdf(doc: jsPDFWithAutoTable, avaliacao: Avaliacao, dims: PdfDimensions): Promise<void> {
  ...
  const doughnutChart = new Chart(canvas1, {
    type: 'doughnut',
    data: {
  ...
  await delay(500)
  const img1 = canvas1.toDataURL('image/png', 1)
  ...
  doc.text((avaliacao.altura ?? 0) > 0 ? avaliacao.altura!.toFixed(1) : 'N/A', margin + (cardWidth + 4) * 2 + 3, cardsY + 12)
  ...
  const foldsChart = new Chart(canvas2, {
    type: 'bar',
    data: {
  ...
  await delay(500)
  const img2 = canvas2.toDataURL('image/png', 1)
  foldsChart.destroy()
  ...
  const tableData1 = [
    ['Peso', `${avaliacao.peso.toFixed(2)} kg`, 'Altura', (avaliacao.altura ?? 0) > 0 ? `${avaliacao.altura!.toFixed(1)} cm` : 'N/A'],
    ['Gordura %', `${avaliacao.percentualGordura.toFixed(2)}%`, 'Massa Magra', `${avaliacao.massaMagraKg.toFixed(2)} kg`],
    ['Massa Gorda', `${avaliacao.massaGorduraKg.toFixed(2)} kg`, 'IMC', (avaliacao.altura ?? 0) > 0 ? `${(avaliacao.peso / ((avaliacao.altura! / 100) ** 2)).toFixed(2)}` : 'N/A'],
  ]
